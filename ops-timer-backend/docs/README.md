...
MCP 服务器 v2.0 提供 **80 个专用工具**，100% 覆盖后端所有 API，智能体通过一个 JSON 配置即可获得全部能力。
...
| **项目** | `project_list` `project_get` `project_create` `project_update` `project_delete` `project_units` `project_budget_stats` |
...
| **笔记** | `note_list` `note_search` `note_get` `note_create` `note_update` `note_delete` |
| **笔记分组** | `note_group_list` `note_group_create` `note_group_update` `note_group_delete` |
